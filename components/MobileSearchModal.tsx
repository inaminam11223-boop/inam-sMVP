...
const MobileSearchModal: React.FC<MobileSearchModalProps> = ({ isOpen, onClose }) => {
  ...
  return (
    <div className="fixed inset-0 z-[100] bg-white flex flex-col animate-in fade-in slide-in-from-bottom-2 duration-300">
      {/* Search Header */}
      <div className="p-4 border-b border-slate-100 flex items-center gap-2">
        <button 
          onClick={onClose} 
          className="p-3 text-slate-500 hover:bg-slate-100 rounded-2xl active:scale-90 transition-all"
          aria-label="Back"
        >
          <ArrowLeft size={24} />
        </button>
        ...
        <div className="flex-1 relative">
          ...
          <input
            ...
            className="w-full bg-slate-100 border-none rounded-[1.25rem] py-4 pl-11 pr-11 text-sm font-bold placeholder:text-slate-400 focus:ring-2 focus:ring-green-500 outline-none transition-all shadow-inner"
          />
          {query && (
            <button 
              onClick={() => {
              ...
              className="absolute right-3 top-1/2 -translate-y-1/2 p-1.5 bg-slate-200 text-slate-500 rounded-full hover:bg-slate-300 active:scale-90 transition-all"
      ...
      <div className="flex-1 overflow-y-auto bg-slate-50/40 p-5 space-y-4">
        {isLoading && suggestions.length === 0 ? (
          <div className="flex flex-col items-center justify-center py-20 gap-4">
             <Loader2 size={40} className="animate-spin text-green-500 opacity-20" />
             <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Scanning KHAN Network...</p>
          </div>
        ) : query.length < 2 ? (
          <div className="text-center py-20 space-y-6">
            <div className="w-24 h-24 bg-white shadow-xl rounded-[2rem] flex items-center justify-center mx-auto text-green-500 border border-slate-50 animate-bounce">
               <Search size={40} />
            </div>
            <div>
              <p className="font-black text-slate-900 uppercase tracking-tighter text-xl italic leading-none">KHAN BUSSINESS</p>
              <p className="text-sm text-slate-400 font-bold mt-2 leading-relaxed">Search across Peshawar, Lahore, and Karachi.<br/>Enter 2+ characters to begin.</p>
            </div>
            
            <div className="pt-8">
               <p className="text-[10px] font-black text-slate-400 uppercase tracking-[0.2em] mb-4">Trending Tags</p>
               <div className="flex flex-wrap justify-center gap-2">
                 {['Grocery', 'Basmati', 'Tikka', 'Logistics', 'Real Estate'].map(term => (
                   <button 
                    key={term}
                    onClick={() => setQuery(term)}
                    className="px-5 py-2.5 bg-white border border-slate-200 rounded-2xl text-xs font-black uppercase tracking-widest text-slate-600 hover:border-green-500 hover:text-green-600 transition-all shadow-sm"
                   >
                     {term}
                   </button>
                 ))}
          ...
          <div className="text-center py-24 px-8">
            <p className="font-black text-slate-900 uppercase italic tracking-tighter text-lg leading-none">No Results Found</p>
            <p className="text-sm text-slate-400 font-bold mt-2">Try searching for products like 'Rice' or 'Fruit'.</p>
          </div>
        ) : (
          <div className="space-y-3 pb-20">
             <div className="flex items-center justify-between px-2">
                <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Network Results</p>
